package com.circuitguard.ai.usermanagement.populator;

import com.circuitguard.ai.usermanagement.dto.OrganizationDTO;
import com.circuitguard.ai.usermanagement.model.OrganizationModel;
import com.circuitguard.commonservice.enums.ERole;
import org.springframework.stereotype.Component;

@Component
public class OrganizationPopulator {

    public void populate(OrganizationModel source, OrganizationDTO target) {
        if (source == null || target == null) return;

        target.setId(source.getId());
        target.setName(source.getName());
        target.setDescription(source.getDescription());
        target.setDomainName(source.getDomainName());
        target.setActive(source.getActive());

        // Populate admin user
        source.getUsers().stream()
                .filter(u -> u.getRoles().stream().anyMatch(r -> r.getName() == ERole.ROLE_BUSINESS_ADMIN))
                .findFirst()
                .ifPresent(admin -> {
                    target.setAdminUsername(admin.getUsername());
                    target.setAdminFullName(admin.getFullName());
                    target.setAdminPrimaryContact(admin.getPrimaryContact());
                    target.setAdminPassword(admin.getPassword());
                    target.setEmail(admin.getEmail());
                });

    }

}
